...
export default function DatabaseConnection({ dbType = 'mysql', onConnectionChange }) {
  ...
  const dbConfig = {
    mysql: {
      ...
      icon: '🐬',
      defaultPort: '3306',
      endpoint: API_ENDPOINTS.executeMySQL || `${API_ENDPOINTS.generateSQL.replace('/generate-sql', '')}/execute-mysql`
    },
    postgresql: {
      name: 'PostgreSQL',
      icon: '🐘',
      defaultPort: '5432',
      endpoint: API_ENDPOINTS.executePostgres
    }
  }

  const currentDb = dbConfig[dbType]
  
  const [config, setConfig] = useState({
    host: 'localhost',
    port: currentDb.defaultPort,
    database: '',
    user: '',
    password: '',
    ssl: false
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setIsOpen(false)
        setTestResult({ success: true, message: `Connected to ${currentDb.name}!` })
  ...
  const disconnect = async () => {
    ...
    try {
      await fetch(currentDb.endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'disconnect',
      ...
      if (onConnectionChange) {
        onConnectionChange({ connected: false, connectionId: null, dbType })
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="flex items-center gap-2">
        {isConnected ? (
          ...
          >
            <span>{currentDb.icon}</span>
            Connect to {currentDb.name}
          </button>
        )}
      </div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-bold">{currentDb.name} Connection</h2>
                  <p className="text-sm text-neutral-medium-gray">Connect to your {currentDb.name} database</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="form-control">
                  ...
                  <input
                    ...
                    placeholder={currentDb.defaultPort}
                    className="input input-bordered"
                  />
                </div>
